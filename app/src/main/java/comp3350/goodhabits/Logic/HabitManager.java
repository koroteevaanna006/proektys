...
public class HabitManager{
    public static boolean test=false;//Установите значение true во время тестирования
    ...
    public static boolean addHabit(Habit habit){
        if(!test) {
            //Установите уведомление для этой привычки
    ...
    {
        if(!test) {
            //Отмените уведомление на прежнее время
            Notifier.cancelAlarm(habit.getId());
            //Установите уведомление на новое время
            Notifier.setHabitNotification(habit);
        }
    ...
    public static String[] getAllHabitNames() throws Exception{
        ...
        if(habitList.size() == 0)
            throw new Exception("ОШИБКА: хранилище привычек пусто");
    ...
    public static boolean deleteHabit(Habit habit){
        if(!test) {
            //Отмените уведомление об удаленной привычке
            Notifier.cancelAlarm(habit.getId());
        }
